...
(function() {
	...
	function _newAlert(msg,callback) {
		return __box({
            title: '确认框',
            content: msg,
            btns: ['重命名',"更新"],
            callback: function(listen) {
                listen("重命名", function(close, serialize) {
                    close();
                });
                listen("更新",function(close, serialize) {
                	callback();
                	close();
                });
            }
    ...
    function _pcOrmobile(msg,callback) {
		return __box({
            title: '选择',
            content: msg,
            btns: ['PC端',"移动端"],
            callback: function(listen) {
                listen("PC端", function(close, serialize) {
                    callback();
                    close();
                });
                listen("移动端",function(close, serialize) {
                	close();
                });
            }
        }).find('.jquery-layer-btn-0').addClass('jquery-layer-btn-blue').end();
